...
public class CustDetDAOImpl implements CustDetDAOI {
	...
	public List<AndroidPushMsg> getAndroidPushMsgRegIdList() {
		...
		final List<AndroidPushMsg> custList = session.createQuery(
				"FROM AndroidPushMsg").list();
		return custList;
	}
	@Override
	...
	public List<FdbkQues> getFeedbackQuestionsList()
	{
		 final Session session = sessionF.getCurrentSession();
		 final Query query = session
		    .createQuery("From FdbkQues c where c.fdbkQuesF=:flag");
	...
	public List<EorderClientDetail> getClientDetails()
	{
		final Session session = sessionF.getCurrentSession();
		 final Query query = session
		    .createQuery("From EorderClientDetail ");
	...
	public Pincode getPincodeId(int pincodeId) {
		final Session session = sessionF.getCurrentSession();
		final Query query = session
				.createQuery("From Pincode c where c.id=:pincodeId");
	...
	public CustDet login(String mobile, String password) {
		final Session session = sessionF.getCurrentSession();
		final Query query = session
				.createQuery("From CustDet c where c.cntcNo=:mobile and c.custPw=:password");
	...
	public AndroidPushMsg getAndroidPushMsgByEmail(String name) {
		final Session session = sessionF.getCurrentSession();
		final AndroidPushMsg custList = (AndroidPushMsg) session.createQuery("FROM AndroidPushMsg where email=:id")
				.setParameter("id", name).list().get(0);
		return custList;
	}
	@Override
	...
	return criteria.list();*/
		Session session = sessionF.getCurrentSession();
        Criteria criteria = session.createCriteria(Area.class).createAlias("city", "city")
        		.add(Restrictions.like("city.id", cityId));
        criteria.setProjection(
			Projections
					...
					.add(Projections.property("longt"),
							"longt")
					.add(Projections.property("addressDesc"),
							"addressDesc")
				)
				.setResultTransformer(Transformers.aliasToBean(AreaOut.class)).setCacheable(true);
        return criteria.list();
	...
	public List<SingleDayRelt> getOrderByDate(String date)
	{
		  ...
		  final Session session = sessionF.getCurrentSession();
		  final Query query = session
		    .createQuery("From SingleDayRelt where menuDate=:date");
	...
	public List<CityOut> getCityList()
	{
		Session session = this.sessionF.getCurrentSession();
	     ...
	     //   ownerCriteria.add(Restrictions.eq("cityId", cityId));
	        ...
	        criteria.add(Restrictions.like("cityF", 1))
	                //.add(Restrictions.like("cityDesc", cityname))
	       // criteria.add(Property.forName("catgId").in(ownerCriteria))
	        .setProjection(
					Projections
							.projectionList()
	...
	@Override
	 public CustDet loginMobile(String email) {
	  final Session session = sessionF.getCurrentSession();
	  final Query query = session
	    .createQuery("From CustDet c where c.emailId=:email");
	  query.setParameter("email", email);
	...
	public CustDet checkloginMobile(String email, String mobile) {
		// TODO Auto-generated method stub
		 final Session session = sessionF.getCurrentSession();
		  final Query query = session
		    .createQuery("From CustDet c where c.emailId=:email or c.cntcNo=:mobile");
		  query.setParameter("email", email);
		  query.setParameter("mobile", mobile);
          if (query.list().size() == 0)
		   return null;
	...
	public CustDet loginUser(String email,String pwd)
	{
		  final Session session = sessionF.getCurrentSession();
		  final Query query = session
		    .createQuery("From CustDet c where c.emailId=:email and c.custPw=:pwd");
		  query.setParameter("email", email);
		  query.setParameter("pwd",pwd);
	...
	public Address checkAddeFl(int  id)
	{
		  final Session session = sessionF.getCurrentSession();
		  final Query query = session
		    .createQuery("From Address c where c.addrflg=:flag");
		  query.setParameter("flag", 0);
	...
	public List<PkgSizTyp> listOfFood()
	 {
		final Session session = sessionF.getCurrentSession();
		@SuppressWarnings("unchecked")
		final List<PkgSizTyp> cities = session.createQuery("From PkgSizTyp").list();
		return cities;
