...
public class CouponController {
    ...
    public Map<String, Object> applyCoupon(@RequestBody final ApplyCoupon cupn) {
        ...
        List<Cupn> couponCodeDetails = cust.getCouponCodeDetails(cupn);
//        System.out.println("couponSize: " + couponCodeDetails.size());//This Line is commented by Viofixer as a fix for Poor Logging Practice Use of a System Output Stream 
        if (couponCodeDetails.size() > 0) {
            for (Cupn cupndet : couponCodeDetails) {
                ...
                //                System.out.println("stardate>>>" + couponStartDate);//This Line is commented by Viofixer as a fix for Poor Logging Practice Use of a System Output Stream 
                System.out.println("enddate>>>" + couponEndDate);
                System.out.println("todayDate>>>" + todayDate);
                ...
                if (Integer.parseInt(cupndet.getCupnMinAmnt()) <= cupn.getTotalAmount()) {
                    if ((todayDate.after(couponStartDate)) && (todayDate.before(couponEndDate))) {
                        ...
                        //					coupondetails.put("status", "your coupon code has expired");//This Line is commented by Viofixer as a fix for Poor Logging Practice Use of a System Output Stream 
